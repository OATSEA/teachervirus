...
$adminfile = SITE_URL.'tv/admin/OATSEA-tv-admin-osfm/index.php';
$tbcolor1c = "#bacaee";
$tbcolor2c = "#daeaff";
$tbcolor3c = "#7080dd";
$bgcolor1c = "#ffffff";
...
if ((isset($_COOKIE['user']) && $_COOKIE['user'] != $user) || (isset($_COOKIE['pass']) && $_COOKIE['pass'] != md5($pass))) {
...
/****************************************************************/
function home() {
  global $folder, $tbcolor1, $tbcolor2, $tbcolor3, $filefolder, $HTTP_HOST,$adminfile;
  maintop("Home");
  ...
  for ($a=1; $a<count($content1)+1;$a++) {
    ...
    echo isset($content1[$a]) ? $content1[$a] : '';
  }
...
function viewframe($file) {
  global $sitetitle, $folder, $HTTP_HOST, $filefolder,$adminfile;  
  if ($filefolder == "/") {
